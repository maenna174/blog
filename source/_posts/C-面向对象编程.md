...
int Person::m_A = 10;//A::member就表示类A中的成员member
int Person::m_B = 10;
...
public:
    ...
    static void func()
    {
        cout << "func调用" << endl;
        m_A = 100;
        //m_B = 100; //错误，静态成员函数 不可以访问非静态成员变量
    }
...
```C++
//空对象占用内存空间为：1字节
//C++编译器会给每个空对象发分配一个字节内存空间，是为了区分空对象占内存的位置
...
void test01() {
    ...
    const Person person; //在对象前面加const，变为常量对象  
